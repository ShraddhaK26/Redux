...
import { DecrementQty, IncreamentQty, RemoveItem } from '../redux/cardSlice';
...
font-semibold border-2 border-green-400 text-xl '>
    <button className='w-[30%] h-full bg-white flex justify-center hover:bg-gray-200 items-center
     text-green-400' onClick={()=>qty > 1 ? dispatch(DecrementQty(id)): 1}>-</button>
    <span className='w-[40%] h-full bg-slate-200 flex justify-center items-center text-green-400 '>{qty}</span>
   <button className='w-[30%] h-full bg-white flex justify-center hover:bg-gray-200 items-center
    text-green-400' onClick={()=>dispatch(IncreamentQty(id))}>+</button>
</div>
</div>
      </div>
      <div>
